...
CronExpression._isValidConstraintChar = function _isValidConstraintChar(
  constraints,
  value
) {
  ...
  function parseSequence(val) {
    ...
    function handleResult(result) {
      if (result instanceof Array) {
        ...
        // Check constraints
        if (
          Number.isNaN(numResult) ||
          numResult < constraints.min ||
          numResult > constraints.max
        ) {
          throw new Error(
            'Constraint error, got value ' +
              result +
  ...
  function parseRange(val, repeatInterval) {
    ...
    if (atoms.length > 1) {
      ...
      if (Number.isNaN(repeatIndex) || repeatIndex <= 0) {
        throw new Error(
          'Constraint error, cannot repeat at every ' + repeatIndex + ' time.'
        );
      }
...
CronExpression.prototype._findSchedule = function _findSchedule(reverse) {
  ...
  while (stepCount < LOOP_LIMIT) {
    ...
    if (isLInExpressions(this.fields.dayOfWeek)) {
      dayOfWeekMatch =
        dayOfWeekMatch || isLastWeekdayOfMonthMatch(this.fields.dayOfWeek);
    }
    var isDayOfMonthWildcardMatch =
      this.fields.dayOfMonth.length >=
      CronExpression.daysInMonth[currentDate.getMonth()];
    var isDayOfWeekWildcardMatch =
      this.fields.dayOfWeek.length ===
      CronExpression.constraints[5].max - CronExpression.constraints[5].min + 1;
    var currentHour = currentDate.getHours();
...
CronExpression.prototype.stringify = function stringify(includeSeconds) {
  var resultArr = [];
  for (
    var i = includeSeconds ? 0 : 1, c = CronExpression.map.length;
    i < c;
    ++i
  ) {
  ...
  function parse(expression, options) {
    ...
    for (var i = 0, c = CronExpression.map.length; i < c; ++i) {
      ...
      if (i < start || !value) {
        ...
        fields.push(
          CronExpression._parseField(field, val, CronExpression.constraints[i])
        );
      }
    }
 ...
 * @return {Object}
 */
CronExpression.fieldsToExpression = function fieldsToExpression(
  fields,
  options
) {
  ...
  for (var i = 0, c = CronExpression.map.length; i < c; ++i) {
    ...
    if (values.length !== copy.length) {
      throw new Error(
        'Validation error, Field ' + field + ' contains duplicate values'
      );
    }
